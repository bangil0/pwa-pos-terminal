...
const TaxList: React.FC<TaxListProps> = ({ taxes, actions }) => {
  const list = taxes.filter((taxes) => !taxes.isDeleted);

  // Delete selected tax record
  const handleOnClickOnDeleteBtn = (event: React.MouseEvent, taxId: string) => {
    event.preventDefault();
    event.stopPropagation();
    actions.taxes.remove(taxId);
  };
  ...
  const renderBody = (
    <Fragment>
      {list.length ? (
        list.map((taxes) => renderItem(taxes))
      ) : (
